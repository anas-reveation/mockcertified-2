    ...
    <div class="container py-5">
      <div class="head">
        <h1 class="text-center fw-bold font_family_dmsans">
          Still Not <span class="span_blue_color">Convinced</span>?
        </h1>
        <div class="my-4 input_container">
          <input class="font_family_dmsans" type="text" v-model="notCovincedText" />
          <i class="fa-solid fa-magnifying-glass search_btn"></i>
        </div>
      </div>

      <div class="row align-items-center">
        <div data-aos="fade-up" v-for="(data, i) in notConvincedData" :key="i" class="col-12 col-md-7">
...
export default {
  ...
  async mounted() {
    const query = groq`*[_type == "notConvinced"] | order( _updatedAt asc){ inputTitle, images }`;
    this.notConvincedData = await this.$sanity.fetch(query);
    this.notCovincedText = this.notConvincedData[0]?.inputTitle;
    this.isFetched = true;
  },
};
</script>
